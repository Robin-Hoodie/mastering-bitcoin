...
export function main() {
  ...
  const privateKeyWithVersionPrefix = Buffer.concat([Buffer.alloc(1, 0x80), privateKey]);
  const privateKeyWithVersionPrefixCompressed = Buffer.concat([privateKeyWithVersionPrefix, Buffer.alloc(1, 0x01)]);
  console.log("Private key - without version prefix - (hex) is ", privateKey.toString("hex"));
  ...
  console.log("Public key x (hex) value is ", publicKeyHexWithoutPrefix.slice(0, publicKeyHexWithoutPrefix.length / 2));
  const publicKeyHash = ripemd160(hash256(publicKey));
  const publicKeyHashWithVersionPrefix = Buffer.concat([Buffer.alloc(1, 0x00), publicKeyHash]);
  console.log("Uncompressed address is ", bs58check.encode(publicKeyHashWithVersionPrefix));
